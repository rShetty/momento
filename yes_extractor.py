"""YES Bank credit card statement specific extractor.

Handles common YES Bank statement layouts where:
  - Vertical table layout (label above, value below)
  - ₹ may render as "C" in some PDF engines
  - Dates in DD MMM YYYY or DD-MM-YYYY format
  - Card masks: XXXX-XXXX-XXXX-1234
  - Password is usually first 4 of name + last 4 of card
"""
...
# YES Bank-specific keyword variations
_YES_AMOUNT_LABELS = [
    "total amount due",
    "total outstanding",
    "amount due",
...
def parse_yes_statement(raw_text: str) -> dict[str, Any] | None:
    """
    Extract all fields from a YES Bank credit card statement.
    Returns a dict with at least due_date and amount on success.
    """
    lines = raw_text.splitlines()
    result: dict[str, Any] = {"bank_key": "yes"}

    # ── 1. Amount ───────────────────────────────
    for label in _YES_AMOUNT_LABELS:
        val = extract_next_line_value(raw_text, label)
        if val:
            # YES Bank may also render ₹ as "C"
            val = val.lstrip("C")
    ...
    # ── 2. Due Date ─────────────────────────────
    for label in _YES_DUE_DATE_LABELS:
        val = extract_next_line_value(raw_text, label)
        if val:
            dt = parse_date(val)
            if dt:
    ...
    # ── 3. Statement Date ───────────────────────
    for label in _YES_STATEMENT_DATE_LABELS:
        val = extract_next_line_value(raw_text, label)
        if val:
            dt = parse_date(val)
            if dt:
    ...
    # ── 4. Bill Cycle ───────────────────────────
    for label in _YES_CYCLE_LABELS:
    ...
    if "bill_cycle" not in result:
        # YES Bank format: "Statement Period: 18 Mar 2026 to 17 Apr 2026"
        m = re.search(
    ...
    # YES Bank-specific: "Card No.: XXXX-XXXX-XXXX-1234"
    ...
    # ── 6. Card Name ────────────────────────────
    for line in lines:
        if "yes bank" in line.lower() and "credit card" in line.lower():
